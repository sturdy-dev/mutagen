...
func TestIgnoreSturdy_IgnoreVSC(t *testing.T) {
	test := &ignoreTestCase{
		ignores: []string{
			"*",
			"!*",
...
func TestIgnoreSturdy_IgnoreAllWithExceptions(t *testing.T) {
	test := &ignoreTestCase{
		ignores: []string{
			"*",
			"!README.md",
		...
		tests: []ignoreTestValue{
			...
			{"db/file", false, false},
			{"db/dir/file", false, false},
			{"directory/subdirectory/file.txt", false, false},
			{"directory/subdirectory", true, true},
		},
	}
	test.run(t)
}
